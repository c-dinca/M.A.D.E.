...
[NFR-025](../03-requirements.md) requires a tested restore drill. Concurrent
writers from multiple worker processes — the first scaling step in
[02-architecture.md](../02-architecture.md) — mean writer
...
([NFR-021](../03-requirements.md)) for a performance problem that
...
([02-architecture.md](../02-architecture.md)) are enforceable rather than aspirational.
...
[NFR-020](../03-requirements.md). Postgres unavailability stops all
execution, by design ([02-architecture.md](../02-architecture.md)), which
...
Event append latency exceeds [NFR-017](../03-requirements.md) at the target
